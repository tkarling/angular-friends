myApp.controller('FriendController', function($scope) {
    ...
    $scope.$watch('search.current_location.$', function(newval, oldval) {
        if ($scope.search && $scope.search.current_location.$ === "") {
            $scope.search = undefined;
        }
        // console.log('search.current_location.$', $scope.search);
    });

    $scope.$watch('sort', function(newval, oldval) {
        $scope.reverse = $scope.options[0];
    });
